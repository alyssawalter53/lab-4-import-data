...
library(viridis) #for the plot color theme
```
...
```{r}
#| label: load-data
buoy_temps<- read_table("data-raw/buoy_2024.txt")
```
...
buoy_temps<- read_table("data-raw/buoy_2024.txt", comment = "#")
```
...
```{r}
#| label: clean-names
buoy_temps_clean<-clean_names(buoy_temps)
```
...
write_csv(buoy_temps_clean, "data-raw/buoy_update.csv")
```
...
updated_buoy_data<- read_csv("data-raw/buoy_update.csv", col_types = cols(.default = "c"), na = c("99", "999"))

spec(updated_buoy_data)
...
```{r}
#| label: parse-double
buoy_final <- updated_buoy_data |> 
  mutate(across(.cols = everything(), ~parse_double(.x)))
  
# writing clean data to file
write_csv(buoy_final, "data-clean/buoy_final.csv")
```
...
ggplot(data = buoy_final, aes(x = mm, y = atmp, color = factor(mm)))+
  labs(title = "Monterey Bay Caynon Air Temperature in 2024",
...
```
> To match this data visualization I referenced statology.org to learn how to display all months on the x axis: https://www.statology.org/ggplot2-scale_x_continuous/
...
> The dataset I have chosen consists of the top songs on spotify in 2020, and I obtsained the dataset from Tidy Tuesdsy. Variables include song information such as track_name, track_artist, track_popularity (a numeric rank out of 100),  playlist_genre, and various other factors such as danceability and energy.

>Potential Research Questions:

>1. What playlist genere had the most songs in the spotify top 100 in 2020?

>2. What artist had the most songs in the spotify top 100 in 2020?

>3. Are there any factors that make a song more likely to end up in the top 100?

>link:https://github.com/rfordatascience/tidytuesday/tree/main/data/2020/2020-01-21
